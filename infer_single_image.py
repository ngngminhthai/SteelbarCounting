...
IMAGE_PATH = Path(r"C:\Users\User\Downloads\00000815429100048316.Image.235000-3.jpg")
OUTPUT_IMAGE_PATH = Path(r"C:\Users\User\Downloads\CrowdCounting-P2PNet\single_image_result.jpg")
OUTPUT_JSON_PATH = Path(r"C:\Users\User\Downloads\CrowdCounting-P2PNet\single_image_result.json")
...
@torch.no_grad()
def predict_points(model, image: Image.Image, device):
    image_tensor = preprocess_pil(image).to(device)
...
def main():
    ...
    points, scores = predict_points(model, original_image, device)
    ...
    print(f"Image size: {original_image.width}x{original_image.height}")
